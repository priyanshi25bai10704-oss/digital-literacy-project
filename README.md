# digital-literacy-project - CSE0001
CSE0001 digital literacy project vit bhopal
*Student Digital Ambassador Portfolio*

*Submitted by:* Priyanshi  
*Reg No.:* 25BAI10704 
*Branch & Year:** btech cse aiml 2025| 1st Year  
*VIT Bhopal University*

## Project Setup & How to View 

### Step-by-step instructions to explore the project:
1.⁠ ⁠Open this repository in any web browser.
2.⁠ ⁠Click on each folder below to view its contents.
3.⁠ ⁠All screenshots, files, and the Project Report are inside the folders.
4.⁠ ⁠The full written report is in the ⁠ report/ ⁠ folder (Project_Report.docx or .pdf).
5.⁠ ⁠No dependencies or environment setup needed — just open the files.

## Repository Structure (as per project brief)
•⁠  ⁠⁠ task-1-presentation/ ⁠ → Digital Literacy Awareness Infographic
•⁠  ⁠⁠ task-2-portfolio/ ⁠ → Screenshots of GitHub, LinkedIn & Kaggle profiles
•⁠  ⁠⁠ task-3-platforms/ ⁠ → HackerRank challenge + Google Form Quiz
•⁠  ⁠⁠ task-4-email-etiquette/ ⁠ → Professional email drafts + Social Media checklist
•⁠  ⁠⁠ task-5-cybercrime/ ⁠ → Phishing case study + Prevention checklist
•⁠  ⁠⁠ report/ ⁠ → Full Project_Report.docx/pdf

## Google Form (Task 3 - Digital Literacy Awareness Quiz)


All work follows the official CSE0001 project brief. Repository is Public as required.
